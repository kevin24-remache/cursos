document.addEventListener("DOMContentLoaded", async function () {
    ...
    function reloadSection() {
        let id_pago = idRuta;
        try {
            fetch(`${base_url}punto/pago/getDatosPgDeposito/${id_pago}`).then(
                (response) => {
                    if (!response.ok) {
                        throw new Error(
    ...
    function updateDeposito(rowIndex, oldData) {
        ...
        fetch(`${base_url}punto/pago/actualizarEstado`, {
            method: 'POST',
            headers: {
    ...
    $('#depositosTable tbody').on('click', 'tr', function () {
        ...
        $('#imagen-deposito').attr('src', base_url + imagenUrl);
    });
    ...
    $("#formRechazo").on("submit", function (e) {
        ...
        fetch(`${base_url}punto/pago/verificarDepositoIncompleto/${paymentId}`)
            .then(response => response.json())
            .then(data => {
                if (data.status === 'success') {
    ...
    $("#formSoloRechazo").on("submit", function (e) {
        ...
        // Realizar la verificación antes de enviar el formulario
        fetch(`${base_url}punto/pago/verificarDepositoRechazado/${paymentId}`)
            .then(response => response.json())
            .then(data => {
